package com.netcracker.edu.varabey.inventory.springutils.beanpostprocessor;
...
@Component
public class LoggedAnnotationHandlerBeanPostProcessor implements BeanPostProcessor {
    protected final Logger logger = LoggerFactory.getLogger(getClass());

    protected final Map<String, Class> annotatedClazzMap = new HashMap<>();

    /* for not suspicious logging */
    protected final Map<Class<?>, Logger> loggerClazzMap = new HashMap<>();
    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(Logged.class)) {
                ...
                loggerClazzMap.put(originalBeanClazz, LoggerFactory.getLogger(originalBeanClazz));
    ...
    protected class Delegate {
        ...
        private String buildMessage(Class<?> originalBeanClazz, Method method, String logMessage) {
            messageBuilder.setLength(0);
            messageBuilder
//                    .append(originalBeanClazz.getSimpleName())
//                    .append(" -> ")
...
//                    .append(" : ")
        ...
        protected void logBefore(Class<?> originalBeanClazz, Method method, Logged log) {
            ...
            logMessage(buildMessage(originalBeanClazz, method, log.messageBefore()), log.level(), loggerClazzMap.get(originalBeanClazz));
        }
        ...
        protected void logAfter(Class<?> originalBeanClazz, Method method, Logged log) {
            ...
            logMessage(buildMessage(originalBeanClazz, method, log.messageAfter()), log.level(), loggerClazzMap.get(originalBeanClazz));
        }

        protected void logMessage(String message, LogLevel level, Logger logger) {
            switch (level) {
                case WARN:
                    logger.warn(message);
                    break;
